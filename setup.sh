...
function checkLatestRelease(){
    ...
    latest_url=$(curl -fsSLI --retry 3 --connect-timeout 15 -o /dev/null -w "%{url_effective}" "https://github.com/$GH_REPO/releases/latest" 2>/dev/null)
    latest_release=$(echo "$latest_url" | sed -n 's#.*/tag/\([^/?#]*\).*#\1#p' | head -n 1)

    if [ -z "$latest_release" ]; then
        release_payload=$(curl -fsSL --retry 3 --connect-timeout 15 -H "Accept: application/vnd.github+json" "https://api.github.com/repos/$GH_REPO/releases/latest" 2>/dev/null)
        if command -v jq &> /dev/null; then
            latest_release=$(echo "$release_payload" | jq -r '.tag_name // empty' 2>/dev/null)
        fi
        if [ -z "$latest_release" ]; then
            latest_release=$(echo "$release_payload" | sed -n 's/^[[:space:]]*"tag_name"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' | head -n 1)
        fi
    fi
...
function getFallbackDownloadUrl(){
    if [ -n "$APP_RELEASE" ] && [ "$APP_RELEASE" != "stable" ] && [ "$APP_RELEASE" != "local" ]; then
        echo "https://raw.githubusercontent.com/$GH_REPO/$APP_RELEASE/deployments/cli/community"
    else
...
function validateDownloadedFile(){
    ...
    if [ ! -s "$FILE_PATH" ]; then
        echo "Downloaded $FILE_TYPE file is empty." >&2
        return 1
    fi

    case "$FILE_TYPE" in
        compose)
            grep -Eq '^[[:space:]]*services:' "$FILE_PATH" && grep -Eq 'plane-(frontend|backend|proxy|space|admin|live)' "$FILE_PATH"
            VALID=$?
            ;;
        env)
            grep -Eq '^APP_RELEASE=' "$FILE_PATH"
            VALID=$?
            ;;
    esac

    if [ "$VALID" -ne 0 ]; then
        echo "Downloaded $FILE_TYPE file did not pass validation." >&2
        return 1
    fi
...
function downloadWithValidation(){
    local URL=$1
    local OUTPUT_FILE=$2
    local FILE_TYPE=$3
    local HTTP_STATUS
    ...
    rm -f "$OUTPUT_FILE"
    HTTP_STATUS=$(curl -fsSL --retry 3 --connect-timeout 15 -H 'Cache-Control: no-cache, no-store' -o "$OUTPUT_FILE" -w "%{http_code}" "$URL")
    if [ $? -ne 0 ]; then
    ...
    if [ "$HTTP_STATUS" != "200" ]; then
        echo "Failed to download $URL. HTTP Status: $HTTP_STATUS" >&2
        rm -f "$OUTPUT_FILE"
        return 1
    fi

    validateDownloadedFile "$OUTPUT_FILE" "$FILE_TYPE"
}

function downloadPlaneFile(){
    local FILE_NAME=$1
    local OUTPUT_FILE=$2
    local FILE_TYPE=$3
    local CACHE_BUSTER
    ...
    CACHE_BUSTER=$(date +%s)
    PRIMARY_URL="$RELEASE_DOWNLOAD_URL/$APP_RELEASE/$FILE_NAME?$CACHE_BUSTER"
    FALLBACK_BASE_URL=$(getFallbackDownloadUrl)
    FALLBACK_URL="$FALLBACK_BASE_URL/$FILE_NAME?$CACHE_BUSTER"

    echo "Downloading $FILE_NAME for Plane $APP_RELEASE..." >&2
    if downloadWithValidation "$PRIMARY_URL" "$OUTPUT_FILE" "$FILE_TYPE"; then
        return 0
    fi

    echo "Trying fallback download for $FILE_NAME..." >&2
    if downloadWithValidation "$FALLBACK_URL" "$OUTPUT_FILE" "$FILE_TYPE"; then
        return 0
    fi

    if [ "$FALLBACK_BASE_URL" != "$FALLBACK_DOWNLOAD_URL" ]; then
        echo "Trying $BRANCH fallback download for $FILE_NAME..." >&2
        if downloadWithValidation "$FALLBACK_DOWNLOAD_URL/$FILE_NAME?$CACHE_BUSTER" "$OUTPUT_FILE" "$FILE_TYPE"; then
            return 0
        fi
    fi

    echo "Failed to download a valid $FILE_NAME for Plane $APP_RELEASE." >&2
    return 1
}
...
function download() {
    local LOCAL_BUILD=$1
    local TMP_DIR
    local TMP_DOCKER_FILE_PATH
    local TMP_DOCKER_ENV_PATH
    local TS

    cd "$SCRIPT_DIR" || exit 1
    TS=$(date +%s)
    TMP_DIR=$(mktemp -d)
    TMP_DOCKER_FILE_PATH="$TMP_DIR/docker-compose.yml"
    TMP_DOCKER_ENV_PATH="$TMP_DIR/variables.env"

    if ! downloadPlaneFile "docker-compose.yml" "$TMP_DOCKER_FILE_PATH" "compose"; then
        rm -rf "$TMP_DIR"
        exit 1
    fi

    if ! downloadPlaneFile "variables.env" "$TMP_DOCKER_ENV_PATH" "env"; then
        rm -rf "$TMP_DIR"
        exit 1
    fi
    ...
    then
        mv "$DOCKER_FILE_PATH" "$PLANE_INSTALL_DIR/archive/$TS.docker-compose.yaml"
    fi
    ...
    mv "$TMP_DOCKER_FILE_PATH" "$DOCKER_FILE_PATH"
    mv "$TMP_DOCKER_ENV_PATH" "$DOCKER_ENV_PATH"
    rm -rf "$TMP_DIR"
